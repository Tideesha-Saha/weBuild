...
function Experience({enabledNext}) {
  const [experienceList, setExperienceList] = useState([]); // Initialize as an empty array
  ...
  // FIX: Initialize experienceList from resumeInfo only once on mount
  useEffect(() => {
    // Check if resumeInfo.Experience exists and has data
    if (Array.isArray(resumeInfo?.Experience) && resumeInfo.Experience.length > 0) {
      setExperienceList(resumeInfo.Experience);
    } else {
      // If no experience data in context, start with one empty form field
      setExperienceList([formField]);
    }
  }, []); // Empty dependency array means this runs only once on mount

  // This useEffect correctly updates resumeInfo whenever experienceList changes
  useEffect(() => {
    setResumeInfo({
      ...resumeInfo,
      Experience: experienceList,
    });
  }, [experienceList]); // Dependency array: run when experienceList changes
  ...
  const removeNewExperience = () => {
    // Ensure you don't remove the last entry if you want at least one
    if (experienceList.length > 1) {
      setExperienceList((experienceList) => experienceList.slice(0, -1));
    } else {
      // Optionally, reset the last entry if it's the only one
      setExperienceList([formField]);
    }
  };
