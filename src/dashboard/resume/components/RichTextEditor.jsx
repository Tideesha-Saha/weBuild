...
export default function RichTextEditor({ onRichTextEditorChange, index, value }) { // Receive 'value' as a prop
  const { resumeInfo, setResumeInfo } = useContext(ResumeInfoContext);
  const [loading,setLoading]=useState(false);
  const [haveClickedAIBtn, setHaveClickedAIBtn]=useState(false);

  // No internal 'value' state needed here for the editor content itself.
  // The 'value' prop will control the editor content.
  ...
  const GenerateSummaryFromAI = async () => {
    ...
    try {
      const result = await AIChatSession.sendMessage(prompt);
      const resp = result.response.text();
      const cleanResp = resp.replace("```html", "").replace("```", "");
      
      // Instead of setting local state, call the parent's change handler
      // This will update the parent's state directly
  ...
  return (
    <div>
        ...
        >
          {loading && haveClickedAIBtn ? (
            ...
            <svg
              ...
              className="size-5 mr-2"
            >
              <path d="M15.98 1.804a1 1 0 0 0-1.96 0l-.24 1.192a1 1 0 0 1-.784.785l-1.192.238a1 1 0 0 0 0 1.962l1.192.238a1 1 0 0 1 .785.785l.238 1.192a1 1 0 0 0 1.962 0l.238-1.192a1 1 0 0 1-.785-.785l-1.192-.238a1 1 0 0 0 0-1.962l-1.192-.238a1 1 0 0 1-.785-.785l-.238-1.192ZM6.949 5.684a1 1 0 0 0-1.898 0l-.683 2.051a1 1 0 0 1-.633.633l-2.051.683a1 1 0 0 0 0 1.898l2.051.684a1 1 0 0 1 .633.632l.683 2.051a1 1 0 0 0 1.898 0l.683-2.051a1 1 0 0 1 .633-.633l2.051-.683a1 1 0 0 0 0-1.898l-2.051-.683a1 1 0 0 1-.633-.633L6.95 5.684ZM13.949 13.684a1 1 0 0 0-1.898 0l-.184.551a1 1 0 0 1-.632.633l-.551.183a1 1 0 0 0 0 1.898l.551.183a1 1 0 0 1 .633.633l.183.551a1 1 0 0 0 1.898 0l.184-.551a1 1 0 0 1 .632-.633l.551-.183a1 1 0 0 0 0-1.898l-.551-.184a1 1 0 0 1-.633-.632l-.183-.551Z" />
            </svg>
          )}
          Generate from AI
        </Button>
      ...
      <EditorProvider>
        <Editor
          containerProps={{ style: { resize: "vertical" } }}
          value={value} // Now using the prop 'value'
          onChange={(e) => {
            onRichTextEditorChange(e); // Simply pass the event up
          }}
        >
          <Toolbar>
            <BtnUndo />
